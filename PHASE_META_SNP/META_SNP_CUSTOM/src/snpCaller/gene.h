...
//==========================================================================
//Define the bit masks for recovering each base from integer
int baseMask[10] = {939524096,117440512,14680064,1835008,229376,28672,3584,448,56,7};
//Define the 3 bit to base mapping and reverse
char intToBase[5] = {'A','T','C','G','N'};

const std::pair<char,short> baseToShort[] = {
  std::make_pair('A',0),std::make_pair('T',1),std::make_pair('C',2),std::make_pair('G',3),std::make_pair('N',4)
};
//==========================================================================
/**
...
public:
  ...
  Genome(std::string seq) {//Get this giant string into a manageble form!
    ...
    sequence = new unsigned int[(length/10)+1];
    unsigned int repr;
    int pos = 0;
	...
	pos = 0;
      ...
      repr = repr | (map_baseToShort[seq[i]] << 3*pos);
    }
  ...
  /**
     @breif Return sequence between coordinates, including end
   */
